...
- **クリックできない要素にホバー時のハイライト（ボーダー色変化・シャドウ等）は付けない。** ホバーエフェクトはリンク・ボタン等のインタラクティブ要素にのみ使用します。
...
| 要素 | モバイル | デスクトップ (md以上) |
|---|---|---|
| リンク一覧（NoLogic, NextCreators等） | 縦並び (`flex-col`) gap-4 | 横並び (`flex-row`) gap-6 |
| CTAボタン（3つ） | 統一幅 `min-w-[220px]` + `justify-center` | 横並び wrap |

### 8-4. ナビゲーションのレスポンシブ対応
...
|---|---|
| 2026-02-26 | 初版作成 |
| 2026-03-10 | フッターレスポンシブ対応を追記（リンク縦並び、CTAボタン幅統一） |
| 2026-03-10 | ホバールール明文化（クリック不可要素にホバーハイライト禁止） |
