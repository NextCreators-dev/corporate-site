...
Astro + Tailwind CSS + React で構築された静的サイトで、netlifyを使って https://creatorpot.net にデプロイされている。
...
| パス | ファイル | 内容 |
|------|---------|------|
| `/` | `index.astro` | トップページ（Hero, News, Track Record, Product, Community, Services, Vision, Company） |
| `/essay` | `essay.astro` | 代表メッセージ |
| `/techplantstudio` | `techplantstudio.astro` | TechPlant Studio サービス詳細 |
| `/news` | `news/index.astro` | ニュース一覧 |
| `/news/[slug]` | `news/[slug].astro` | ニュース個別記事 |
| `/privacy` | `privacy.astro` | プライバシーポリシー |
...
│   │   ├── TrustSection.astro   # 実績セクション
│   │   ├── PortfolioCard.astro  # ポートフォリオカード
...
| `Footer.astro` | `hideCta?: boolean` | フッター。`hideCta=true` で CTA セクションを非表示。SPではリンク縦並び、CTAボタン幅統一（min-w-[220px]） |
| `TrustSection.astro` | - | 実績セクション（Track Record） |
| `PortfolioCard.astro` | - | ポートフォリオカード |
...
| `src/pages/news/[slug].astro` | `/news/:slug` |
...
7. **ニュースセクションのCMS化**
   - 現状: `src/data/news.ts` にデータを定義し、`/news` 一覧ページ・`/news/[slug]` 個別ページで表示（動的ルーティング実装済み）
   - 改善案: Markdown/MDX ファイルや外部 CMS と連携し、非エンジニアでもコンテンツ更新可能にする

8. ~~**プライバシーポリシーページの作成**~~ → **実装済み**
   - `/privacy` ページを作成し、フッターからリンク済み
